...
2. `update`
   Для новых батчей считаются Data Quality-метрики и ассоциативные правила.
3. `train`
...
- `update`  
  Загружает датасет, выполняет батчирование, сохраняет сырые данные в SQLite, считает Data Quality и association rules для новых батчей.
- `train`  
